...
RunService.Heartbeat:Connect(function()
    if State.jobs.tracking and not State.jobs.paused then local now=os.clock(); local last=State.jobs.lastTick or now; State.jobs.elapsed+=math.max(0,now-last); State.jobs.lastTick=now
...
local plotTick=0
RunService.Heartbeat:Connect(function(dt)
    plotTick+=dt or 0; if plotTick<1 then return end; plotTick=0
