...
local BUILD = "v1.4-runtime-patch"
...
local adapterWork=card(jp,"Live Work Detection ♡","Kimbugers now watches Bloxburg's confirmed job-efficiency UI. Task automation is not labeled working until we have a captured job-task transition.")
local liveWork=infoLine(adapterWork,"Work state","Not detected",true)
local liveEff=infoLine(adapterWork,"Efficiency","—",true)
local liveAdapter=infoLine(adapterWork,"Auto Work adapter",Adapters.Jobs and "Connected ✓" or "Not connected",Adapters.Jobs~=nil)
button(adapterWork,"REFRESH WORK STATUS",function()
    local working,pct=runtimeJobEfficiency()
    liveWork.Text=working and "Working ♡" or "Not detected"
    liveEff.Text=pct or "—"
    notify(working and ("Work detected  "..tostring(pct or "")) or "No active work UI detected.")
end)
RunService.Heartbeat:Connect(function()
    if not liveWork.Parent then return end
    local working,pct=runtimeJobEfficiency()
    liveWork.Text=working and "Working ♡" or "Not detected"
    liveEff.Text=pct or "—"
    liveAdapter.Text=Adapters.Jobs and "Connected ✓" or "Not connected"
end)
